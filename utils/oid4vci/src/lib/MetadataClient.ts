import { getIssuerFromCredentialOfferPayload } from './functions/CredentialOfferUtil';
import { getJson } from './functions/HttpUtils';
import {
...
export class MetadataClient {
  ...
  public static async retrieveAllMetadataFromCredentialOffer(
    ...
    return MetadataClient.retrieveAllMetadataFromCredentialOfferRequest(
  ...
  public static async retrieveAllMetadataFromCredentialOfferRequest(
    ...
    throw new Error(
      "can't retrieve metadata from CredentialOfferRequest. No issuer field is present",
    );
  }
  ...
  public static async retrieveAllMetadata(
    ...
    const oid4vciResponse =
      await MetadataClient.retrieveOpenID4VCIServerMetadata(issuer, {
    ...
    if (credentialIssuerMetadata) {
      ...
      if (credentialIssuerMetadata.authorization_endpoint) {
        authorization_endpoint =
          credentialIssuerMetadata.authorization_endpoint;
      }
    }
    // No specific OID4VCI endpoint. Either can be an OAuth2 AS or an OIDC IDP. Let's start with OIDC first
    let response: OpenIDResponse<AuthorizationServerMetadata> =
      await MetadataClient.retrieveWellknown(
    ...
    if (authMetadata) {
      console.log(
        `Issuer ${issuer} has OpenID Connect Server metadata in well-known location`,
      );
      authorizationServerType = 'OIDC';
    } else {
      // Now let's do OAuth2
      response = await MetadataClient.retrieveWellknown(
        authorization_server,
    ...
    if (!authMetadata) {
      ...
      if (issuer !== authorization_server) {
        throw Error(
          `Issuer ${issuer} provided a separate authorization server ${authorization_server}, but that server did not provide metadata`,
        );
      }
    } else {
      ...
      console.log(
        `Issuer ${issuer} has ${authorizationServerType} Server metadata in well-known location`,
      );
      if (!authMetadata.authorization_endpoint) {
        ...
        authorization_endpoint &&
        authMetadata.authorization_endpoint !== authorization_endpoint
      ) {
        throw Error(
      ...
      if (!authMetadata.token_endpoint) {
        throw Error(
          `Authorization Sever ${authorization_server} did not provide a token_endpoint`,
        );
      } else if (
        token_endpoint &&
        authMetadata.token_endpoint !== token_endpoint
      ) {
        throw Error(
      ...
      if (authMetadata.credential_endpoint) {
        if (
          credential_endpoint &&
          authMetadata.credential_endpoint !== credential_endpoint
        ) {
          console.log(
    ...
    if (!authorization_endpoint) {
      console.log(
        `Issuer ${issuer} does not expose authorization_endpoint, so only pre-auth will be supported`,
      );
    }
    if (!token_endpoint) {
      console.log(
        `Issuer ${issuer} does not have a token_endpoint listed in well-known locations!`,
      );
    ...
    if (!credential_endpoint) {
      console.log(
        `Issuer ${issuer} does not have a credential_endpoint listed in well-known locations!`,
      );
      if (opts?.errorOnNotFound) {
    ...
    console.log(
      `Issuer ${issuer} token endpoint ${token_endpoint}, credential endpoint ${credential_endpoint}`,
    );
    return {
      issuer,
      token_endpoint,
  ...
  public static async retrieveOpenID4VCIServerMetadata(
    ...
    return MetadataClient.retrieveWellknown(
      issuerHost,
      WellKnownEndpoints.OPENID4VCI_ISSUER,
      {
        errorOnNotFound:
          opts?.errorOnNotFound === undefined ? true : opts.errorOnNotFound,
      },
    );
  }
  ...
  public static async retrieveWellknown<T>(
    ...
    const result: OpenIDResponse<T> = await getJson(
      `${host.endsWith('/') ? host.slice(0, -1) : host}${endpointType}`,
      {
    ...
    if (result.origResponse.status >= 400) {
      ...
      console.log(
        `host ${host} with endpoint type ${endpointType} status: ${result.origResponse.status}, ${result.origResponse.statusText}`,
      );
    }
    return result;
  }
